...
<title>Fundus Stage1.5 · Six-Lesion Monitor</title>
<style>
...
<header><div class="head"><div><div class="title">Fundus Qwen3-VL · Stage1.5 Six-Lesion</div><div class="sub">Limited six-lesion adaptation · balanced Main-4 · exploratory IRMA/NV · automatic evaluation</div></div><div><div class="live"><span class="dot"></span><span id="status">CONNECTING</span></div><div class="sub mono" id="clock"></div></div></div></header>
<main>
...
<section class="grid two">
 ...
 <div class="panel"><div class="ph"><h2>Run Configuration</h2><span class="badge cyan">Stage1.5 limited</span></div><div class="kv" id="config"></div></div>
</section>
<section class="grid three">
 <div class="panel"><div class="ph"><h2>Stage1.5 Distribution</h2><span class="badge">5,680 total rows</span></div><div id="dist"></div></div>
...
def parse_log(path: Path, started: float, root: Path) -> dict[str, Any]:
    ...
    processes = run(["pgrep", "-fc", "llamafactory-cli train.*stage1_5_six_lesion_limited.yaml"])
    running = int(processes or 0) > 0
    metrics = root / "saves/qwen3-vl-8b-fundus/lora/stage1_eval/stage1_5_six_lesion/gold_test/stage1_metrics.json"
    ...
    phase = "gold_test scored" if completed else "training completed; checkpoint evaluation" if training_completed and eval_running else "training completed" if training_completed else "Stage1.5 optimizer steps" if step else "tokenizing / loading model" if running else "not running"
...
def parse_evaluation(root: Path) -> dict[str, Any]:
    eval_log_dir = root / "logs/stage1_5_six_lesion/eval"
...
def dataset_state(stats_path: Path) -> dict[str, Any]:
    ...
    for lesion in ("MA", "HE", "EX", "SE", "IRMA", "NV"):
        ...
        positive = item.get("present", {}).get("rows", 0)
        negative = item.get("absent", {}).get("rows", 0)
        dist.append({"lesion": lesion, "positive": positive, "negative": negative})
        tiers.update(item.get("present", {}).get("tiers", {}))
        tiers.update(item.get("absent", {}).get("tiers", {}))
    ...
    evals = [
        ...
        {"name": "Hard negatives", "rows": sum(v.get("absent", {}).get("hard_negatives", 0) for v in selection.values()), "purpose": "specificity calibration"},
        {"name": "Base replay", "rows": stats.get("total_rows", 0), "purpose": "preserve Stage1 F1"},
    ]
...
def main() -> None:
    ...
    args = ap.parse_args()
    root = args.root
    full_log = root / "logs/stage1_5_six_lesion/train.log"
    smoke_log = root / "logs/stage1_5_six_lesion/train.log"
    log = full_log if full_log.exists() else smoke_log
    stats = root / "data/annotation_v4/fundus_stage1_5_six_lesion_stats.json"
    config_path = Path("/workspace/fundus-qwen3vl-project/configs/train/stage1_5_six_lesion_limited.yaml")
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self):  # noqa: N802
            if self.path.startswith("/api/state"):
                ...
                payload = {"now": time.time(), "train": parse_log(log, started, root), "evaluation": parse_evaluation(root), "gpu": gpu(), "memory": memory(), "dataset": dataset_state(stats), "config": {"Base model": cfg["model_name_or_path"], "LoRA": f"r={cfg['lora_rank']}, alpha={cfg['lora_alpha']}, dropout={cfg['lora_dropout']}", "Vision tower": "LoRA enabled", "Projector": "trainable", "Learning rate": cfg["learning_rate"], "Effective batch": cfg["per_device_train_batch_size"] * cfg["gradient_accumulation_steps"], "Epochs": cfg.get("num_train_epochs", f"max_steps={cfg.get('max_steps', '?')}"), "Image max pixels": cfg["image_max_pixels"], "Precision": "bf16", "Eval set": "Gold Dev + rare locked final"}, "health": {"train_processes": int(run(["pgrep", "-fc", "llamafactory-cli train.*stage1_5_six_lesion_limited.yaml"]) or 0), "eval_processes": parse_evaluation(root)["processes"], "log_size_mb": round(log.stat().st_size / 1048576, 2) if log.exists() else 0, "output_exists": str((root / cfg["output_dir"]).exists()), "disk_free_gb": round(disk.free / 1073741824, 1), "hostname": socket.gethostname(), "public_bind": f"{args.host}:{args.port}"}, "log_path": str(log), "tail": tail(log)}
