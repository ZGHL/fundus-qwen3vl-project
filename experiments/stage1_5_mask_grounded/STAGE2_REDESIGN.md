...
### 1. 模型的感知边界：能可靠看 4 种病灶，其余看不了
- **可靠**：经 Stage-1.5（v3 已达 macro BalAcc 0.742，v5 进一步抬 HE 召回 / MA·SE 特异度），模型能可靠感知 **MA、HE、EX、SE** 四种病灶的 present/absent。
- **不可靠**：**NV（新生血管）、IRMA**——数据天花板（全数据集真 mask 仅 49 / 159），视觉本就难分辨（锁定评测 F1 ≈ 0.21 / 0.33），且 RetSAM 也不产出 → **无法可靠识别**。
- **根本看不到**：ICDR 分级还依赖**逐象限微动脉瘤计数（4-2-1 规则）**和**静脉串珠（venous beading）**等——VLM 数不准小而多的病灶（v2 已证伪），静脉串珠无标注 → 这些证据**对模型不可得**。

### 2. 因此无法做到完整、可信的 Grade 0–4 分级
标准 ICDR 0–4 分级所需的证据，**大半要么不可靠（NV/IRMA）、要么根本看不到（象限计数、静脉串珠）**。硬把模型训成 0–4 分级器，必然让它在**看不见的证据上猜**——既不可信，也破坏可解释性（这正是旧 Stage-2 失败的根源）。

### 3. 因此按「忠实性」重新分类：可识别的→映射，不可识别的→弃权
我们不强行分 0–4，而是**只用模型能可靠看到的 4 病灶**，经一张透明映射得到忠实分诊；**凡是依赖不可靠/不可见证据的判断，一律显式弃权**：

| 类别 | 处置 |
|---|---|
| MA / HE / EX / SE 的有无 | ✅ 可识别 → 进入审计与映射 |
| NV / IRMA | ❌ 不可靠 → **永远弃权**（绝不当可见证据，防臆造增殖性病变） |
| 逐象限计数 / 静脉串珠 | ❌ 不可得 → 不使用；相关的严重度细分以弃权表达 |
| Moderate vs Severe 仅凭可见病灶不可分的边界 | ⚠️ → **校准弃权**（转诊但标"严重度待专科判定"） |

→ 这套"能看的就忠实判、看不清的就诚实弃权"的取舍，正是下面整套方案（标签空间、决策树、弃权、数据、CoT）的出发点。
