...
def main():
    ...
    # TEST images — IDENTICAL selection to v3/v4 (same params + hash keys) -> comparable
    ...
    for name, rows in [("stage1_5_v5_train", train), ("stage1_5_v5_test", test)]:
    ...
    meta = {"train": st(train), "test": st(test), "pres_cap": PRES_CAP, "abs_cap": ABS_CAP,
            ...
            "note": "v5 per-lesion rebalance: HE present-push (recall), MA/SE absent-push (specificity), "
                    "EX unchanged; SE/MA absent rank confounder-rich DR images first; real masks only, "
                    "NO grade-derived/RetSAM-SE weak labels; Stage-2 test stems excluded (leak-free); "
                    "same single-lesion format + Adapter1 warm-start + v3/v4-identical test"}
